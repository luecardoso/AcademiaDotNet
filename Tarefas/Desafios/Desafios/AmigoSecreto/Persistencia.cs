...
namespace AmigoSecreto
{
    internal class Persistencia
    {
        ...
        public static void editar(string nomeArquivo, string email)
        {

        }

        public static void remover(string nomeArquivo, string email, List<Pessoa> lista)
        {
            int cont = 0;
            try
            {
                ...
                do
                {
                    ...
                    //}
                    //if (vetorLinha[1] == email)
                    //{
                    //    if (lista.Contains(email))
                    //   {

                    //  }
                    //MessageBox.Show("Removendo" + vetorLinha[1] +" | email"+email, "Informação", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    //linha = String.Empty;
                    //escritor.WriteLine(vetorLinha[0] + ";" + vetorLinha[1]);
                    // escritor.Flush();
                    //  lista.Remove(cont);
                    //lista.Clear();
                    gravarListaArquivo(lista, "amigos.csv");
                    //escritor.Close ();
                    //}
                } while (!leitor.EndOfStream);
                leitor.Close();
        ...
        //public static void removerPessoa(string nomeArquivo, string email, List<Pessoa> lista)
        //{
        //    StreamWriter escritor = new StreamWriter(nomeArquivo);
        //    escritor.WriteLine(vetorLinha[0] + ";" + vetorLinha[1]);
        //    escritor.Flush();
